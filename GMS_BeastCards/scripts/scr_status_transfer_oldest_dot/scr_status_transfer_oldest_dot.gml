//===============================================================================//
//
// SCRIPT: SCR_STATUS_TRANSFER_OLDEST_DOT
...
//
// ARGUMENTS: _ref_source is the Beast losing its oldest DoT and _ref_target is
//            the Beast receiving that DoT.
// RETURNS: True when a DoT is successfully transferred; otherwise false.
//
//===============================================================================//

function scr_status_transfer_oldest_dot(_ref_source,_ref_target){
	...
	//========================//
	var _it_source_status = ds_list_find_index(_ref_source._list_statuses,_ref_dot);

	if (_it_source_status == -1){
		return false;
	}

	var _str_dot_name = _ref_dot._str_status_name;
	var _ct_stacks = max(1,_ref_dot._ct_status_stacks);

	//==========================//
	//CHECK MATCHING TARGET DOT//
	//==========================//
	var _ref_target_dot = scr_status_check(_str_dot_name,_ref_target);

	if (
		_ref_target_dot != -1 &&
		!instance_exists(_ref_target_dot)
	){
		_ref_target_dot = -1;
	}
	...
	switch (_str_dot_name){
		...
		case "VENOM":
			...
			if (is_struct(_ref_source._ref_unit)){
				...
				_ref_source._ref_unit._val_beast_ppow_stat += _ref_dot._val_venom_ppow_reduction;
				_ref_source._ref_unit._val_beast_mpow_stat += _ref_dot._val_venom_mpow_reduction;
				_ref_source._ref_unit._val_beast_pdef_stat += _ref_dot._val_venom_pdef_reduction;
				_ref_source._ref_unit._val_beast_mdef_stat += _ref_dot._val_venom_mdef_reduction;
			}

			_ref_dot._val_venom_ppow_reduction = 0;
			_ref_dot._val_venom_mpow_reduction = 0;
			_ref_dot._val_venom_pdef_reduction = 0;
			_ref_dot._val_venom_mdef_reduction = 0;
	...
	//========================//
	ds_list_delete(_ref_source._list_statuses,_it_source_status);
	...
	if (_ref_target_dot != -1){
		...
		//MERGE STACKS//
		//----------------//
		_ref_target_dot._ct_status_stacks += _ct_stacks;
		...
		if (
			...
			_ref_target_dot._val_status_lifetime = max(
				_ref_target_dot._val_status_lifetime,
				_ref_dot._val_status_lifetime
			);

			_ref_target_dot._val_status_lifetime_max = max(
		...
		switch (_str_dot_name){
			...
			case "VENOM":
				...
				if (is_struct(_ref_target._ref_unit)){
					...
					_ref_target._ref_unit._val_beast_ppow_stat = max(0,_val_old_ppow - _val_stat_reduction);
					_ref_target._ref_unit._val_beast_mpow_stat = max(0,_val_old_mpow - _val_stat_reduction);
					_ref_target._ref_unit._val_beast_pdef_stat = max(0,_val_old_pdef - _val_stat_reduction);
					_ref_target._ref_unit._val_beast_mdef_stat = max(0,_val_old_mdef - _val_stat_reduction);

					_ref_target_dot._val_venom_ppow_reduction += _val_old_ppow - _ref_target._ref_unit._val_beast_ppow_stat;
					_ref_target_dot._val_venom_mpow_reduction += _val_old_mpow - _ref_target._ref_unit._val_beast_mpow_stat;
					_ref_target_dot._val_venom_pdef_reduction += _val_old_pdef - _ref_target._ref_unit._val_beast_pdef_stat;
					_ref_target_dot._val_venom_mdef_reduction += _val_old_mdef - _ref_target._ref_unit._val_beast_mdef_stat;
				}
		...
		//------------------------//
		instance_destroy(_ref_dot);
	}

	//==========================//
	//MOVE STATUS TO NEW HOST//
	//==========================//
	else{
		...
		//CHANGE HOST//
		//-------------//
		_ref_dot._ref_host = _ref_target;
		_ref_dot._ref_status_target = _ref_target;
		_ref_dot._str_status_command = "WAIT";
		...
		switch (_str_dot_name){
			...
			case "VENOM":
				...
				if (is_struct(_ref_target._ref_unit)){
					...
					_ref_dot._val_venom_ppow_reduction = _val_old_ppow - _ref_target._ref_unit._val_beast_ppow_stat;
					_ref_dot._val_venom_mpow_reduction = _val_old_mpow - _ref_target._ref_unit._val_beast_mpow_stat;
					_ref_dot._val_venom_pdef_reduction = _val_old_pdef - _ref_target._ref_unit._val_beast_pdef_stat;
					_ref_dot._val_venom_mdef_reduction = _val_old_mdef - _ref_target._ref_unit._val_beast_mdef_stat;
				}
	...
	scr_gui_spawn_popup_scrolling(
		"TEXT",
		"TRANSFERRED " + string(_ct_stacks) + " " + _str_dot_name,
		undefined,
		c_aqua,
		_ref_target.x,
		_ref_target.y - 48

function scr_status_cc_stun(_tag,_ref){
	switch(_tag){
		case "APPLY":
			_ref = global.target_beast;
			
			var _status = scr_check_unit_status("STUN",_ref);
			if (_status != -1){
				_status._status_lifetime = 1;
				return _status;
			}
	
			//Make a new status obj
			var _new_status = instance_create_layer(_ref.x,_ref.y,"ily_status",obj_battle_status);
			_new_status._status_lifetime = 1;
			_new_status._status_scr = scr_status_cc_stun;			
			_new_status._ref_host = _ref;
			_new_status._status_name = "STUN";
			_new_status._status_desc = "STUNNED, CANNOT MOVE";
			_new_status._status_sprite = spr_status_cc_stun;

//===============================================================================//
//
// SCRIPT: SCR_BATTLE_RESOLVE_DISCHARGE
// FUNCTION: Resolves exactly one available Stormstruck DISCHARGE on a Beast.
//           Consumes floor(current threshold * 0.5) Stormstruck stacks,
//           deals 15 NEU damage to the host, and spreads half the consumed
//           stacks to eligible allies.
//           DISCHARGE itself does not alter Stormstruck lifetime.
//
// ARGUMENTS: _ref_host - Beast attempting one DISCHARGE.
// RETURNS: True if one DISCHARGE resolves; otherwise false.
//
//===============================================================================//
function scr_battle_resolve_discharge(_ref_host){
	...
	//CALCULATE STACKS//
	//==================//
	var _ct_consumed = max(
		1,
		floor(_ct_threshold * 0.5)
	);

	var _ct_spread = max(
		1,
		floor(_ct_consumed * 0.5)
	);
	...
	//CHECK UNSTABLE COIL//
	//=====================//
	var _ref_unstable_coil =
		scr_status_check("UNSTABLE_COIL",_ref_host);
	...
	if (_flag_unstable_coil){
		...
		var _list_allies =
			scr_battle_get_target_team_list(_ref_host);
	...
	for (
		var _it_target = 0;
		_it_target < array_length(_arr_spread_targets);
		_it_target++
	){
		var _ref_target =
			_arr_spread_targets[_it_target];

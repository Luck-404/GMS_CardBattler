...
function scr_card_viridian_seed_the_field(_stct_card,_ref_caster,_ref_target){
	...
	for (var _it_target = 0; _it_target < ds_list_size(_list_targets); _it_target++){
		...
		var _ct_open_slots = max(0,_ref_affected_target._ct_minions_max - ds_list_size(_ref_affected_target._list_minions));

		repeat (_ct_open_slots){
			scr_minion_init("DORMANT_SEED",_stct_card,_ref_caster,_ref_affected_target);
			_ct_seeds_spawned++;
		}
	}

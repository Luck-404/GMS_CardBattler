...
//           Host Attacks may deal NEU splash damage to adjacent enemies.
...
function scr_status_aura_krakens_chosen(_str_tag,_ref_status,_val_magnitude=undefined,_ref_attack_target=undefined){
	...
	switch (_str_tag){
		...
		case "APPLY":
			...
			//CHECK EXISTING//
			//----------------//
			var _ref_existing_status = scr_status_check("KRAKENS_CHOSEN",_ref_target);
			...
			_ref_new_status._scr_status = scr_status_aura_krakens_chosen;

			_ref_new_status._ref_host = _ref_target;

			_ref_new_status._str_status_type = "AURA";
			_ref_new_status._str_status_name = "KRAKENS_CHOSEN";
			_ref_new_status._str_status_desc = "CC IMMUNE; SUMMONS TENTACLES; ATTACKS APPLY STORMSTRUCK";

			_ref_new_status._spr_status = spr_status_aura_krakens_chosen;

			_ref_new_status._ct_status_stacks = 1;

			_ref_new_status._flag_status_cc_immunity = true;

			_ref_new_status._val_hp_loss_percent = 10;

			/*
				NOT DEFINED IN CURRENT CARD DATA.

				Set this when the intended adjacent
				NEU damage value is finalized.
			*/
			_ref_new_status._val_adjacent_damage = undefined;
		...
		case "TRIGGER":
			...
			if (
				...
				for (var _it_target = 0;_it_target < array_length(_arr_adjacent);_it_target++){
					...
					var _ref_adjacent_target = _arr_adjacent[_it_target];

					if (!instance_exists(_ref_adjacent_target)){
						continue;
					}

					if (_ref_adjacent_target._str_team == _ref_host._str_team){
						continue;
					}
		...
		case "REPEAT":
			...
			var _val_actual_hp_loss = min(_ref_host._val_cur_hp,_val_hp_loss);

			_ref_host._val_cur_hp -= _val_actual_hp_loss;

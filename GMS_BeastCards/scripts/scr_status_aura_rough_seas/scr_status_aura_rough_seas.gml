...
function scr_status_aura_rough_seas(_str_tag,_ref_status,_val_magnitude=undefined,_ref_trigger_target=undefined){
	...
	switch (_str_tag){
		...
		case "APPLY":
			...
			for (var _it_beast = 0;_it_beast < ds_list_size(_list_team);_it_beast++){
				...
				var _ref_existing_status = scr_status_check("ROUGH_SEAS",_ref_beast);
			...
			for (var _it_beast = 0;_it_beast < ds_list_size(_list_team);_it_beast++){
				...
				_ref_beast._val_dmg_taken_scalar_bonus += _ref_new_status._val_aura_damage_taken_bonus;
			}
		...
		case "TRIGGER":
			...
			if (_ref_trigger_target._str_team == _ref_status._str_team){
				return false;
			}
			...
			//GET TARGET TEAM//
			//----------------//
			var _list_target_team = (_ref_trigger_target._str_team == "PLAYER") ? obj_battle_player_controller._list_beasts_alive : obj_battle_enemy_controller._list_beasts_alive;

			if (!ds_exists(_list_target_team,ds_type_list)){
				return false;
			}

			var _it_position = ds_list_find_index(_list_target_team,_ref_trigger_target);
			...
			//----------------------//
			var _val_move = _arr_moves[irandom(array_length(_arr_moves) - 1)];

function scr_status_buff_draw_2(_tag,_ref){
	switch(_tag){
		case "APPLY":
			_ref = global.statuses;
			
			var _status = scr_check_unit_status("DRAW_2",global.statuses);
			if (_status != -1){
			...
			_new_status._status_scr = scr_status_buff_draw_2;				
			_new_status._ref_host = undefined;
			_new_status._status_name = "DRAW_2";
			_new_status._status_desc = "+2 CARD DRAW FOR 3 TURNS";
			_new_status._status_sprite = spr_status_buff_draw_2;
			_new_status._trigger_region = "END";
			_new_status._status_type = "GLOBAL";
	
			obj_battle_player_controller._draw_amount += 2;		

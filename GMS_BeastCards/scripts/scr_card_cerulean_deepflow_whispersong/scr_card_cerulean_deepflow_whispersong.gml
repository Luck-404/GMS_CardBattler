...
//           Performs summon/growth attempts equal to the card magnitude.
//           Each attempt randomly summons or grows a Tentacle on the selected team.
//
	...
	repeat (_stct_card._val_card_magnitude){
		...
		for (var _it_beast = 0;_it_beast < ds_list_size(_list_team);_it_beast++){
			...
			if (scr_minion_has_open_slot(_ref_beast)){
				array_push(_arr_valid_hosts,_ref_beast);
				continue;
			}
			...
			//FULL — CHECK TENTACLE//
			//----------------------//
			var _ref_tentacle = scr_minion_get_oldest_by_name(_ref_beast,"TENTACLE");
		...
		//ROLL HOST//
		//----------------//
		var _ref_host = _arr_valid_hosts[irandom(array_length(_arr_valid_hosts) - 1)];
		...
		//=====================//
			...
			for (var _it_minion = 0;_it_minion < ds_list_size(_ref_host._list_minions);_it_minion++){
				...
				array_push(_arr_tentacles,_ref_minion);
			}
			...
			if (array_length(_arr_tentacles) > 0){
				...
				var _ref_growth_target = _arr_tentacles[irandom(array_length(_arr_tentacles) - 1)];

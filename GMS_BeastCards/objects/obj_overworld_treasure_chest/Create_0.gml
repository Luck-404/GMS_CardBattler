...
//—------------------------------------------------------------------------------//
	function hscr_award_custom_treasure_chest_loot(){
		...
		for (var _it_reward = 0; _it_reward < array_length(_arr_rewards); _it_reward++){
			...
			switch(_str_reward_type){
				case "CARD":
					for (var _it_card = 0; _it_card < _val_reward_count; _it_card++){
						var _ref_card = scr_card_get_info(_str_reward_name);
						scr_deck_add_card(_ref_card);
					}
				break;
			...
			scr_gui_spawn_popup("TEXT","+" + _str_reward_name + "x" + string(_val_reward_count),undefined,c_white,obj_player.x + _val_rand_x,obj_player.y + _val_rand_y);
		}
	}
	...
	function hscr_roll_treasure_chest_reward(){
		for (var _it_loot = 0; _it_loot < _val_loot_amount; _it_loot++){
			...
			if (_str_reward_type == "CARD"){
				...
				var _ref_card = scr_card_get_info(_str_card_name);
				...
				scr_gui_spawn_popup("TEXT","+" + _str_card_name,undefined,c_white,obj_player.x + _val_rand_x,obj_player.y + _val_rand_y);
			} else {
				...
				scr_gui_spawn_popup("TEXT","+" + string(_str_new_item),undefined,c_black,obj_player.x + _val_rand_x,obj_player.y + _val_rand_y);
			}	
			...
			scr_gui_spawn_popup("TEXT","+" + string(_val_new_gold) + "gp",undefined,c_yellow,obj_player.x + _val_rand_x,obj_player.y + _val_rand_y);
		}		
	}
#endregion
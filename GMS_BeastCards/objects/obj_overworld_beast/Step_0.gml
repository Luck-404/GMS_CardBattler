...
// Plays nearby beast footsteps while the beast is moving.
//
//===============================================================================//
...
// BATTLE TRIGGER
if (!_flag_battle_triggered && _ct_battle_cooldown <= 0 && place_meeting(x,y,obj_player) && !instance_exists(obj_battle_wait)){
...
var _val_step_dist_player = point_distance(x,y,obj_player.x,obj_player.y);
var _flag_beast_moving = (x != xprevious || y != yprevious);

if (_flag_beast_moving && _val_step_dist_player <= 64 && _ct_step_sound_cooldown <= 0){
	audio_play_sound(snd_overworld_beast_step,0,false);

	_ct_step_sound_cooldown = max(1,round(60 * 0.25));
}

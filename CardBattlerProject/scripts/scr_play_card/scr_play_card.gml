...
function scr_play_card(card_script, target_creature,card_cost) {
	show_debug_message("\n === PLAYING CARDS === \n");
	
	//if echo is in place, play card twice
	if (global.echo == true){		
		if (card_script == scr_card_echo){	
			global.echo_count +=1;
			...
			for (var _j = -1; _j < global.echo_count; _j++){	
				...
				card_script(target_creature);
			}
			global.echo_count = 0;
			global.echo = false;
			//subract cost once
			global.current_mana = global.current_mana - card_cost;
		}
	...
	if (card_script != scr_card_echo){
		...
		if (card_script == scr_card_inspiration){

...
// DEATH CHECK //
/////////////////
if (_minion_hp_cur <= 0 && _flag_has_died == false){
	_flag_has_died = true;
...
if ((global.flag_gui_open == false) && position_meeting(mouse_x,mouse_y,self) && (global.player_enc_state=PLAYER_ENCOUNTER_STATE.PICK_CHANNEL || global.player_enc_state=PLAYER_ENCOUNTER_STATE.PICK_TARGET || global.player_enc_state=PLAYER_ENCOUNTER_STATE.PICK_CARD)){
	...
	draw_text(mouse_x+15, mouse_y+5, _minion_name + " " + string(_minion_cur_hp) + "/" + string(_minion_max_hp));
	draw_text(mouse_x+15, mouse_y+20, "Defense: " + string(_minion_def));	
	draw_text(mouse_x+15, mouse_y+35, _minion_color);	
	draw_text(mouse_x+15, mouse_y+50, _minion_subtype);	
...
var _health_bar_width = 100 * (_minion_hp_cur / _minion_hp_max);
...
draw_set_color(c_white);
draw_text(_bar_x + 5, _bar_y + 3, string(_minion_hp_cur) + "/" + string(_minion_hp_max));
...
draw_set_color(c_white);
draw_text(_defense_x - 4, _defense_y - 5, string(_minion_def));  // Display the defense value inside the circle